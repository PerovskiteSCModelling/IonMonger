...
[chi, delta, G, R, lambda, lam2, Rr, Rl, N, Kn, Kp, NE, lamE2, KE, kE, ...
    ...
    Pm, SEinv, omegE, SHinv, omegH, phidisp, Ptol] ...
...
elseif any(Pm) && strcmp(NonlinearFP,'Diffusion')
    PAP = 0;
    PD = -1/Pm*log1p(Pm*(P(1:N)-P(2:N+1))./(1-Pm*P(1:N)+Pm*Ptol))./dx;
    % with small tolerance Ptol for numerical stability
else
    PAP = 0;
	PD = Dx*P;
end
