...
AvSPS = Av*SPS(P); % averaged limit vacancy operator for steric effects
...
dSPSdP = (SPS(P*(1+del/2))-SPS(P*(1-del/2)))./(P*del); % derivative of the steric statistical function
...
JJJ(1:N+1,1:N+1) = nnz(DI)*lambda*gallery('tridiag',N+1, ...
    -1/2*dEP.*(1-AvSPS - AvP.*dSPSdP(1:end-1))+(AvP.*(1-AvSPS)).*xd.*dSPinvdP(1:end-1), ...
    [1/2*dEP(1).*(1-AvSPS(1) - AvP(1).*dSPSdP(1))-AvP(1)*(1-AvSPS(1))*xd(1)*dSPinvdP(1);
    1/2*dEP(2:end).*(1-AvSPS(2:end) - AvP(2:end).*dSPSdP(2:end-1))-(AvP(2:end).*(1-AvSPS(2:end))).*xd(2:end).*dSPinvdP(2:end-1) - ...
    1/2*dEP(1:end-1).*(1-AvSPS(1:end-1) - AvP(1:end-1).*dSPSdP(2:end-1))- (AvP(1:end-1).*(1-AvSPS(1:end-1))).*xd(1:end-1).*dSPinvdP(2:end-1);
    -1/2*dEP(end).*(1-AvSPS(end) - AvP(end).*dSPSdP(end))-AvP(end)*(1-AvSPS(end))*xd(end)*dSPinvdP(end)], ...
    1/2*dEP.*(1-AvSPS - AvP.*dSPSdP(2:end))+(AvP.*(1-AvSPS)).*xd.*dSPinvdP(2:end));

% P equation depends on phi
JJJ(1:N+1,N+2:2*N+2) = nnz(DI)*lambda*gallery('tridiag',N+1, ...
    AvP.*(1-AvSPS)./dx, ...
    [-AvP(1).*(1-AvSPS(1))./dx(1); -AvP(1:N-1).*(1-AvSPS(1:N-1))./dx(1:N-1)-AvP(2:N).*(1-AvSPS(2:N))./dx(2:N); -AvP(N).*(1-AvSPS(N))./dx(N)], ...
    AvP.*(1-AvSPS)./dx);
