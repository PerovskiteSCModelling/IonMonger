...
dSidnE = (SEinv(omegE*(nE*(1+del/2)))-SEinv(omegE*(nE*(1-del/2)))) ...
            ./(nE*del); % derivative of inverse ETL statistical function
dSidpH = (SHinv(omegH*(pH*(1+del/2)))-SHinv(omegH*(pH*(1-del/2)))) ...
...
JJJ(2*N+3,4*N+2*NE+4:4*N+2*NE+5) = KE*[-1/2*dEnE(end)+AvnE(end)*xdE(end)*dSidnE(end-1),...
    -1/2*dEnE(end)-AvnE(end)*xdE(end)*dSidnE(end)];
...
JJJ(4*N+4,4*N+2*NE+NH+6:4*N+2*NE+NH+7) = KH*[1/2*dEpH(1)-AvpH(1)*xdH(1)*dSidpH(1),...
    1/2*dEpH(1)+AvpH(1)*xdH(1)*dSidpH(2)];
...
JJJ(4*N+NE+5:4*N+2*NE+5,4*N+NE+5:4*N+2*NE+5) = kE*KE*gallery('tridiag',NE+1, ...
    [-1/2*dEnE(1:NE-1)+AvnE(1:NE-1).*xdE(1:NE-1).*dSidnE(1:NE-1); 0], ...
    [1/(kE*KE); -1/2*dEnE(1:NE-1)-AvnE(1:end-1).*xdE(1:NE-1).*dSidnE(2:NE)+...
    1/2*dEnE(2:NE)-AvnE(2:NE).*xdE(2:NE).*dSidnE(2:NE); 0], ...
    [0; 1/2*dEnE(2:NE)+AvnE(2:NE).*xdE(2:NE).*dSidnE(3:NE+1)]);
% JJJ(4*N+2*NE+5,4*N+2*NE+5) = -AE*dSEinvdnE(end)*exp(SEinv(omegE*nE(end)));
JJJ(4*N+2*NE+5,4*N+2*NE+5) = -dSidnE(end)*exp(SEinv(omegE*nE(end))-SEinv(omegE));
...
JJJ(4*N+2*NE+NH+6:4*N+2*NE+2*NH+6,4*N+2*NE+NH+6:4*N+2*NE+2*NH+6) = ...
    ...
    [AvpH(1:NH-1).*xdH(1:NH-1).*dSidpH(1:NH-1)-1/2*dEpH(1:NH-1); 0], ...
    [0; -AvpH(1:NH-1).*xdH(1:NH-1).*dSidpH(2:NH)-1/2*dEpH(1:NH-1)-...
    AvpH(2:NH).*xdH(2:NH).*dSidpH(2:NH)+1/2*dEpH(2:NH); 1/(kH*KH)], ...
    [0; AvpH(2:NH).*xdH(2:NH).*dSidpH(3:NH+1)+1/2*dEpH(2:NH)]);
% JJJ(4*N+2*NE+NH+6,4*N+2*NE+NH+6) = -AH*dSHinvdpH(1)*exp(SHinv(omegH*pH(1)));
JJJ(4*N+2*NE+NH+6,4*N+2*NE+NH+6) = -dSidpH(1)*exp(SHinv(omegH*pH(1))-SHinv(omegH));
...
if nargin>5
    ...
    elseif strcmp(flag,'open-circuit')
        ...
        JJJ(4*N+5,:) = kE*KE*[zeros(1,4*N+4), ...
                              ...
                              zeros(1,NE-2), ...
                              +1/2*dEnE(1)-AvnE(1).*xdE(1).*dSidnE(1),...
                              1/2*dEnE(1)+AvnE(1).*xdE(1).*dSidnE(2), ...
                              zeros(1,NE-2+2*NH+2)];
