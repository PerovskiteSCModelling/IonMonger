...
dSPinvdP = (SPinv(P+del)-SPinv(P))/del; % derivative of the inverse ion statistical function
EnE = SEinv(omegaE*nE)-phiE; % ETL quasi-Fermi level
dEnE = DxE*EnE; % derivative of ETL quasi-Fermi level
dSEinvdnE = (SEinv(omegaE*(nE+del))-SEinv(omegaE*nE))/del;% derivative of the inverse ETL statistical function
EpH = SHinv(omegaH*pH)+phiH; % HTL quasi-Fermi level
dEpH = DxH*EpH;
dSHinvdpH = (SHinv(omegaH*(pH+del))-SHinv(omegaH*pH))/del;% derivative of the inverse ETL statistical function

% P equation depends on P    
JJJ(1:N+1,1:N+1) = nnz(DI)*lambda*gallery('tridiag',N+1, ...
    -1/2*dEP+AvP.*xd.*dSPinvdP(1:end-1), ...
    [1/2*dEP(1)-AvP(1)*xd(1)*dSPinvdP(1);
    1/2*dEP(2:end)-AvP(2:end).*xd(2:end).*dSPinvdP(2:end-1)-1/2*dEP(1:end-1)-AvP(1:end-1).*xd(1:end-1).*dSPinvdP(2:end-1);
    -1/2*dEP(end)-AvP(end)*xd(end)*dSPinvdP(end)], ...
    1/2*dEP+AvP.*xd.*dSPinvdP(2:end));
...
JJJ(4*N+2*NE+NH+6:4*N+2*NE+2*NH+6,4*N+2*NE+NH+6:4*N+2*NE+2*NH+6) = ...
    ...
    [0; -AvpH(1:NH-1).*xdH(1:NH-1).*dSHinvdpH(2:NH)-1/2*dEpH(1:NH-1)-AvpH(2:NH).*xdH(2:NH).*dSHinvdpH(2:NH)+1/2*dEpH(2:NH); 1/(kH*KH)], ...
...
if nargin>5
    ...
    elseif strcmp(flag,'open-circuit')
        ...
        JJJ(4*N+5,:) = kE*KE*[zeros(1,4*N+4), ...
                              ...
                              zeros(1,NE-2), ...
                              +1/2*dEnE(1)-AvnE(1).*xdE(1).*dSEinvdnE(1), 1/2*dEnE(1)+AvnE(1).*xdE(1).*dSEinvdnE(2), ...
                              zeros(1,NE-2+2*NH+2)];
