...
% functions for Steric effects
SPS = @(x) (lim-m(1)*x)./(lim-x); %multiplier for non linear diffusion
SP =  @(x) x.*(1-m(2)*x/lim); %multiplier for non linear dift
AvSPS = Av*SPS(P); % averaged diffusion multiplier
AvSP = Av*SP(P); % averaged drift multiplier
dPdx = Dx*P; %derivative of ion density

% P equation depends on P
JJJ(1:N+1,1:N+1) = nnz(DI)*lambda*gallery('tridiag',N+1, ...
    -1/2*mE.*(1-2*m(2)*AvP/lim) - dPdx*lim*(m(1)-1)./(AvP-lim).^2 + xd.*AvSPS, ...
    [1/2*mE(1).*(1-2*AvP(1)/lim) + dPdx(1)*lim*(m(1)-1)./(AvP(1)-lim).^2 - xd(1).*AvSPS(1);
    -1/2*mE(1:end-1).*(1-2*AvP(1:end-1)/lim) - dPdx(1:end-1)*lim*(m(1)-1)./(AvP(1:end-1)-lim).^2 - xd(1:end-1).*AvSPS(1:end-1) + ...
    1/2*mE(2:end).*(1-2*AvP(2:end)/lim) + dPdx(2:end)*lim*(m(1)-1)./(AvP(2:end)-lim).^2 - xd(2:end).*AvSPS(2:end);
    -1/2*mE(end).*(1-2*AvP(end)/lim) - dPdx(end)*lim*(m(1)-1)./(AvP(end)-lim).^2 - xd(end).*AvSPS(end)], ...
    1/2*mE.*(1-2*AvP/lim) + dPdx*lim*(m(1)-1)./(AvP-lim).^2 + xd.*AvSPS);

% P equation depends on phi
JJJ(1:N+1,N+2:2*N+2) = nnz(DI)*lambda*gallery('tridiag',N+1, ...
    AvSP./(lim*dx), ...
    [-AvSP(1)./(lim*dx(1)); -AvSP(1:end-1)./(lim*dx(1:end-1)) - AvSP(2:end)./(lim*dx(2:end)); -AvSP(end)./(lim*dx(end))], ...
    AvSP./(lim*dx));
...
% n equation depends on pH via Rr
JJJ(3*N+3,4*N+2*NE+NH+6) = -(Rr(n(N+1),pH(1)+del)-Rr(n(N+1),pH(1)))/del;
...
JJJ(3*N+4:4*N+4,3*N+4:4*N+4) = JJJ(3*N+4:4*N+4,3*N+4:4*N+4) ...
    ...
    dx.*(RRp-RR)/del);
% p equation does not depend on phiE
% p equation depends on nE via Rl
JJJ(3*N+4,4*N+2*NE+5) = -(Rl(nE(end)+del,p(1))-Rl(nE(end),p(1)))/del;
