...
JJJ(1:N+1,1:N+1) = lambda*gallery('tridiag',N+1, ...
    xd-mE/2, ...
    [-xd(1)+mE(1)/2; -xd(1:N-1)-mE(1:N-1)/2-xd(2:N)+mE(2:N)/2; ...
        -xd(N)-mE(N)/2], ...
    xd+mE/2);
% P equation depends on phi
JJJ(1:N+1,N+2:2*N+2) = lambda*gallery('tridiag',N+1, ...
    AvP./dx, ...
...
JJJ(2*N+3:3*N+3,2*N+3:3*N+3) = Kn*gallery('tridiag',N+1, ...
    xd+mE/2, ...
    [KE/Kn*(-xdE(end)+mEE(end)/2)/kE-xd(1)-mE(1)/2; ...
...
JJJ(2*N+3:3*N+3,2*N+3:3*N+3) = JJJ(2*N+3:3*N+3,2*N+3:3*N+3) ...
    ...
    dx.*(RRn-RR)/del, ...
    [(dx(1).*(RRn(1)-RR(1))+2*(Rl(nE(end)+del/kE,p(1))-Rl(nE(end),p(1))))/del; ...
...
JJJ(2*N+3:3*N+3,3*N+4:4*N+4) = -1/2*gallery('tridiag',N+1, ...
    ...
    [(dx(1).*(RRp(1)-RR(1))+2*(Rl(nE(end),p(1)+del)-Rl(nE(end),p(1))))/del; ...
        ...
        (dx(N)*(RRp(N)-RR(N))+2*(Rr(n(N+1),pH(1)+del/kH)-Rr(n(N+1),pH(1))))/del], ...
    dx.*(RRp-RR)/del);
...
JJJ(3*N+4:4*N+4,3*N+4:4*N+4) = Kp*gallery('tridiag',N+1, ...
    xd-mE/2, ...
    [-xd(1)+mE(1)/2; -xd(1:N-1)-mE(1:N-1)/2-xd(2:N)+mE(2:N)/2; ...
        -xd(N)-mE(N)/2+KH/Kp*(-xdH(1)+mEH(1)/2)/kH], ...
    xd+mE/2);
...
JJJ(4*N+NE+4,2*N+3) = -dxE(end)/kE/6/lamE2;
% phiE equation does not depend on p
...
JJJ(4*N+2*NE+4,N+2) = -KE*AvnE(end)./dxE(end);
% nE equation depends on n
JJJ(4*N+2*NE+4,2*N+3) = KE*(xdE(end)-mEE(end)/2)/kE;
% nE equation does not depend on p
% nE equation depends on phiE
JJJ(4*N+NE+5:4*N+2*NE+4,4*N+5:4*N+NE+4) = -KE*gallery('tridiag',NE, ...
    AvnE(1:NE-1)./dxE(1:NE-1), ...
    [0; -AvnE(1:NE-1)./dxE(1:NE-1)-AvnE(2:NE)./dxE(2:NE)], ...
    [0; AvnE(2:NE-1)./dxE(2:NE-1)]);
% nE equation depends on nE
JJJ(4*N+NE+5:4*N+2*NE+4,4*N+NE+5:4*N+2*NE+4) = KE*gallery('tridiag',NE, ...
    xdE(1:NE-1)+mEE(1:NE-1)/2, ...
    [1/KE; -xdE(1:NE-1)+mEE(1:NE-1)/2-xdE(2:NE)-mEE(2:NE)/2;], ...
...
JJJ(4*N+2*NE+5,4*N+4) = dxH(1)/kH/6/lamH2;
...
JJJ(4*N+2*NE+NH+5,2*N+2) = KH*AvpH(1)./dxH(1);
% pH equation does not depend on n
% pH equation depends on p
JJJ(4*N+2*NE+NH+5,4*N+4) = KH*(xdH(1)-mEH(1)/2)/kH;
...
JJJ(4*N+2*NE+NH+5:4*N+2*NE+2*NH+4,4*N+2*NE+NH+5:4*N+2*NE+2*NH+4) = ...
    KH*gallery('tridiag',NH, ...
    [xdH(2:NH-1)-mEH(2:NH-1)/2; 0], ...
    [-xdH(1:NH-1)-mEH(1:NH-1)/2-xdH(2:NH)+mEH(2:NH)/2; 1/KH], ...
    xdH(2:NH)+mEH(2:NH)/2);
...
if nargin>5
    ...
    elseif strcmp(flag,'open-circuit')
        ...
        JJJ(4*N+5,:) = [zeros(1,4*N+4), ...
                        KE*AvnE(1)./dxE(1), -KE*AvnE(1)./dxE(1), ...
                        zeros(1,NE-2), ...
                        KE*(-xdE(1)-mEE(1)/2), KE*(xdE(1)-mEE(1)/2), ...
                        zeros(1,NE-2+2*NH)];
