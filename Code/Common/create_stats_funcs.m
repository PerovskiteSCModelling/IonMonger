...
% and 'Gaussian'. Supported distributions are 'Boltzmann' and 
% 'FermiDirac'The Gaussian model requires an extra parameter, 's', giving
% the dimensionless Gaussian disorder of the relevent band. The outputs are
...
if isequal(band, 'parabolic')
    ...
    if ~isempty(s),
        warning(['The Gaussian disorder parameter was defined but was not ' ...
            'used for the parabolic band model.']); end
elseif isequal(band, 'Gaussian')
    if isempty(s)
        error(['The Gaussian band model requires the Gaussian ' ...
...
%     if isequal(distribution , 'distname')
...
%     end
    ...
    error(['Statistical band name not recognised. Choose from <parabolic>',...
        ' or <Gaussian>'])
end

end

% Other functions used in creating statistical integrals:
...
function Ef = stats_interpolate(c_list, Ef_list, c)
    % Interpolation for statistical integrals that gives useful error
    % messages
    if any(c>max(c_list))
